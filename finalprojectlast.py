...
def test():
    return jsonify("Hey i'm testing")
...
def posting():
 if request.method == 'POST':
        ...
        # You can do something with the email/password here
        # For now, let's just return them back for testing
        return jsonify({
            "message": "Data received",
            "average name" : email + password, 
            "email": email,
            "password": password
        }), 200
...
def loginpage():
    if request.method == 'POST':
        ...
        sql = "SELECT * FROM finalproject WHERE email=%s AND password=%s"
        cur.execute(sql, (email, password))
        row = cur.fetchone()        
        #return (row)
        if row:
            usertype = row[3].strip()
            session["usertype"] = usertype
            session["email"] = email
            
            print(usertype, "Check this")
            if usertype == "admin":
                return redirect(url_for("adminhospitals"))
                ...
                # ✅ If usertype doesn't match any role
                return render_template("loginerror.html", error="Unknown user type")
        else:
            # ✅ Wrong credentials
            return render_template("loginerror.html", error="Invalid email or password")

    # ✅ GET request always returns
